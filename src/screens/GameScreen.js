...
const getSuit = (card) => {
	if (card.value === "+" || card.value === "-") return "No Suit";
...
const getNumericVal = (card) => {
	if (card.value === "+" || card.value === "-") return 0;
	const code = card.value.charCodeAt(0);
...
const genPlayerTitle = (num) => {
	if (num == playerNum) {
		return (
			<Text>
				Player {num} <Text style={styles.accent}>(You)</Text>
			</Text>
		);
	}
	return <Text>Player {num}</Text>;
};
...
function genHand(player, num) {
	if (player.hand.contents.length == 0) {
		...
		const cards = player.hand.contents.map((element) =>
			genCard(element, num)
		);
...
const GameScreen = ({ navigation }) => {
	...
	console.log("Game screen rendering for game " + gameCode);
	useEffect(() => {
		console.log("Game Screen useEffect");
	...
	const wordToCode = (str) => {
		...
		if (
			str.toUpperCase() == "FACE UP" ||
			str.toUpperCase() == "UP" ||
			str.toUpperCase() == "FACE"
		) {
			return "P";
		}
		return str;
		...
		return (
			<View style={styles.helpContainer}>
				...
				<Text style={styles.help}>
					<Text style={styles.accent}>Deck:</Text> "Deck", "F"
				</Text>
				<Text style={styles.help}>
					<Text style={styles.accent}>Discard:</Text> "Discard",
					"Dis", "D"
				</Text>
				<Text style={styles.help}>
					<Text style={styles.accent}>Face Up:</Text> "Face Up",
					"Face", "P"
				</Text>
				<Text style={styles.help}>
					<Text style={styles.accent}>Your Hand:</Text> "Hand", "H"
				</Text>
				<Text style={styles.help}>
					<Text style={styles.accent}>Player #'s Hand:</Text> "#"
				</Text>
				<Text style={styles.help}>
					<Text style={styles.accent}>Card:</Text> string of form
					"[Num][Suit]"
				</Text>
				<Text style={styles.help}>
					<Text style={styles.accent}>Example:</Text> 10 of spades :
					10S, Queen of Diamonds : QD
				</Text>
			</View>
		);
	};
	...
	const genControls = () => {
		...
		return (
			<View>
				...
				<Text style={styles.help}>
					Move the top card of{" "}
					<Text style={styles.accent}>souce</Text> (pile) to{" "}
					<Text style={styles.accent}>target</Text> (pile/hand)
				</Text>
				<TouchableOpacity
					style={styles.touchable}
					onPress={() => play()}
				...
				<Text style={styles.help}>
					Move <Text style={styles.accent}>souce</Text> (card) to{" "}
					<Text style={styles.accent}>target</Text> (pile/hand)
				</Text>
				<TouchableOpacity
					style={styles.touchable}
				...
				<Text style={styles.help}>
					Shuffle <Text style={styles.accent}>souce and target</Text>{" "}
					(piles) into deck (if none specified only shuffle deck)
				</Text>
				<TouchableOpacity
					style={styles.touchable}
					onPress={() => flip()}
				...
				<Text style={styles.help}>
					...
					<Text style={styles.accent}>souce</Text> (card) to other
					players
				</Text>
				<TouchableOpacity
					style={
						isDealer() ? styles.touchable : styles.touchableDisabled
					}
					disabled={!isDealer()}
					onPress={() => deal()}
				>
				...
				<Text style={styles.help}>
					Deal <Text style={styles.accent}>souce</Text> (number) cards
					to <Text style={styles.accent}>target</Text> (player #, if
					blank or zero deal to all player)
				</Text>
			</View>
		);
	};
	return (
		<View style={styles.container}>
			...
			<View>
				...
				<TouchableOpacity
					style={
						isHost() ? styles.touchable : styles.touchableDisabled
					}
					disabled={!isHost()}
					onPress={() => kickPlayer()}
				>
