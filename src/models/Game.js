...
exports.Game = exports.PlayerLogData = void 0;
const Rules_1 = require("./Rules");
const Player_1 = require("./Player");
const Card_1 = require("./Card");
const Pile_1 = require("./Pile");
const ReplayObject_1 = require("./ReplayObject");
...
exports.PlayerLogData = PlayerLogData;
class Game {
    constructor(rules) {
        ...
        this.rules = rules ?? new Rules_1.Rules();
        this.deck = new Pile_1.Pile(PileID.DECK);
        this.faceUp = new Pile_1.Pile(PileID.FACEUP, true);
        this.discard = new Pile_1.Pile(PileID.DISCARD, true);
        ...
        if (!rules.withoutHearts)
            for (let i = 65; i <= 77; i++)
                this.deck.insertCard(new Card_1.Card(String.fromCharCode(i)));
        ...
        if (rules.jokersEnabled) {
            this.deck.insertCard(new Card_1.Card('+'));
            this.deck.insertCard(new Card_1.Card('-'));
        }
        this.deck.shuffle();
    ...
    revealCard(args) {
        ...
        if (!found)
            found = playerObj.table.revealCard(cardID);
        if (!found)
            return null;
        return {
    ...
    playerLeave(args) {
        ...
        const playerObj = this.playerState[playerNum - 1];
        if (args[2] === 'K')
            this.blacklistedPlayers.push(playerObj._id);
        ...
        if (playerNum === this.currentDealer) {
            let i = 1;
            for (; i <= 8; i++)
                if (!this.playerState[i - 1].vacant())
                    break;
    ...
    dealCards(args) {
        ...
        if (dealingPlayer !== this.currentDealer)
            return null;
        if (toPlayer === 0) {
            // All players
            ...
            while (this.deck.size() !== 0) {
                ...
                do {
                    ...
                    if (pnum > 8)
                        pnum = 1;
                } while (this.playerState[pnum - 1].vacant() &&
    ...
    async terminateGame() {
        const replayObj = new ReplayObject_1.ReplayObject(this.playerLog, this.deckArrangementLog, this.actionLog, this.rules);
        const replayDoc = new Replay_1.default(replayObj, true);
            ...
            for (let pid of allIDs) {
                try {
                    User_1.default.findOne({
                        ...
                        if (err) {
                            console.log('Could not save replay to user document\n' + err);
                        }
                        else {
                    ...
                    console.log('Adding replay to user failed: not find user with id ' +
                        pid);
                }
            }
        }
    ...
    getPlayerPositionByID(pid) {
        for (let i = 1; i <= 8; i++)
            if (this.playerState[i - 1]._id === pid)
                return i;
        return -1;
    }
    toString() {
